...
export default function ScrollingHeroMarquee({
    ...
    bottomRotateXDeg = -30,
    bottomSkewXDeg = 20,
    lightTopFade = 'transparent',
    lightBottomFade = 'transparent',
    darkTopFade = 'transparent',
    darkBottomFade = 'transparent',
    className = '',
}: ScrollingHeroMarqueeProps) {
    const id = useId().replace(/:/g, '');
    const uniqueClass = `marquee-${id}`;

    return (
        <div
            className={`flex flex-col justify-center ${className} ${uniqueClass}`}
        ...
        >
            ...
            >
                <h2
                    className="relative m-0 font-semibold leading-tight will-change-transform text-foreground"
                    aria-hidden="true"
                    style={{
                        fontSize,
                        top: '100%',
                        animation: `scrollerTop-${id} ${durationSec}s linear infinite`,
                    }}
                >
                    {text}
                </h2>
            ...
            >
                <h2
                    ...
                    style={{
                        fontSize,
                        top: 0,
                        animation: `scrollerBottom-${id} ${durationSec}s linear infinite`,
                    }}
                >
                    {text}
                </h2>
